...
St.add_beam(N1, N2, BLOCKS, D, 0, b=np.pi * D / 4, material=STEEL)
...
d_c = NFC * L / EA_eq
print(4 * d_c * 1e3)
LIST = np.linspace(0, 1e-3, 1000)
# LIST = np.append(LIST, np.linspace(2.5e-3, 1e-2, 5000))
# LIST = np.append(LIST, np.linspace(1e-3, 2e-3, 100))
LIST = LIST.tolist()

St.solve_dispcontrol(LIST, 0, Node, 0, tol=1e-5, filename=f'New_TC_d={D * 1000}mm', max_iter=100)
St.plot_structure(scale=100, plot_cf=False, plot_supp=False, plot_forces=True)

St.save_structure(f'Structure_d={D * 1000}mm')

# %% Plot results

import matplotlib.pyplot as plt

file = f'New_TC_d={D * 1000}mm.h5'
...
# sigma_bs, tau_bs, x = St.get_stresses(tag = 'STC')
# print(sigma_bs)

# St.plot_stresses(angle=np.pi/2)
