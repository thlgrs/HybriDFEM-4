...
with open(f'TwoBlocks_Pulse.pkl', 'rb') as file:
    St = pickle.load(file)

for file_name in os.listdir():

    if file_name.endswith('.25.h5') and file_name.startswith(f'TwoBlocks_{a}g'):
...
for i in range(frames_needed):
    ...
    St.plot_structure(scale=1, plot_cf=True, plot_forces=False, show=False, save=saveto, lims=[[-1., 1.], [-.5, 1.5]])
...
clip.write_videofile(f'Video_TwoBlocks_{a}g.mp4', codec='libx264')
