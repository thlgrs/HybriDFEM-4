...
St1.make_cfs(False, nb_cps=nb_cps, offset=-1, surface=surf.Coulomb(kn, ks, mu), weights=weights)

if model == 'C':
    St1.list_cfs[0].change_cps(nb_cp=[-1, 1], offset=-1, surface=surf.Coulomb(kn, ks, mu), weights=[1 / 2] * 2)
    St1.list_cfs[1].change_cps(nb_cp=[-1, 1], offset=-1, surface=surf.Coulomb(kn, ks, mu), weights=[1 / 2] * 2)
elif model == 'D':
    St1.list_cfs[0].change_cps(nb_cp=[-1, 1], offset=-1, surface=surf.Coulomb(kn, ks, mu), weights=[1] * 2)
    St1.list_cfs[1].change_cps(nb_cp=[-1, 1], offset=-1, surface=surf.Coulomb(kn, ks, mu), weights=[1] * 2)
...
# #%% Computation
Meth = 'CAA'
St1.solve_dyn_nonlinear(2.5, 1e-4, Meth=Meth, lmbda=lmbda, filename=stiff_type + '_' + model + f'_Coulomb_{-a}g_{damp}')
St1.plot_structure(scale=1, plot_forces=False, plot_cf=True)

St1.save_structure(filename=model + '_Arch_Coulomb')
