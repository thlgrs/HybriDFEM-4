...
for i in range(frames_needed):
    plt.figure(None, (6, 5), dpi=400)
    plt.plot(Time[:i * ratio], U_conv[:i * ratio], color='blue')
    plt.plot(t_ls, d_ls, color='red', linestyle=':')
    plt.grid(True)
    plt.xlim((0, 2.5))
    plt.ylim((-15, 20))
...
clip.write_videofile(f'Video_Response_ARCH_{a}g.mp4', codec='libx264')
