...
with open(f'Composite_Frame.pkl', 'rb') as file:
    St = pickle.load(file)

for file_name in os.listdir():

    if file_name.endswith('0.42.h5') and file_name.startswith(f'w={w_s}_a={a}_Full'):
...
for i in range(frames_needed):
    ...
    St.plot_structure(scale=1, plot_cf=False, plot_forces=False, show=False, save=saveto, lims=[[-1., 4.], [-1., 7.]])
...
clip.write_videofile(f'Video_Rocking_{w_s}_{a}.mp4', codec='libx264')
