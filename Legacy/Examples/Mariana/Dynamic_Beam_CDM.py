...
S.make_nodes()
S.make_cfs(lin_geom=True, nb_cps=10)
...
filename = "CA2_Column_Sinusoidal"
...
S.solve_dyn_nonlinear(
    ...
    Meth='CDM',  # or 'GEN', 'CDM', etc. (see ask_method for options)
    filename=filename,
    dir_name=save_path
)
...
with h5py.File(results_file, 'r') as hf:
    ...
    # F_all = hf['P_r_conv'][-3, :]  # X reaction at top node

# === Plot force–displacement curve (publication style)
plt.figure(figsize=(6, 5))
plt.plot(U_all * 1e3, np.array(lambda_list) * Pmax / 1e3, '-', color='black', linewidth=1.2)
